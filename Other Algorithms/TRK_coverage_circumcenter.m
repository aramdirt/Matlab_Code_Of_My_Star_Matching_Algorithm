...
%normal mode
catalog = load('guide_catalogue_in_angle_CSS.txt');
catalog_ID = load('guide_catalogue_ID_CSS.txt');
catalog_TRK = load('catalogue_TRK.txt');
%miss first nearby star mode
catalog_2 = load('guide_catalogue_in_angle_2_CSS.txt');
catalog_ID_2 = load('guide_catalogue_ID_2_CSS.txt');
...
for k = 1:1:loop_count
    ...
    % create initial FOV
    RA_0=boresight(loop_count,7);
    DEC_0=boresight(loop_count,8);
    PHI_R=0;
    ...
    if num_matched_stars +  1 < Min_matching_stars || ~is_match_correct     %num_matched_stars + 1 is mainstar
        ...
        NG_result(NG_cnts,4)=boresight(loop_count,7);
        NG_result(NG_cnts,5)=boresight(loop_count,8);
        NG_result(NG_cnts,6)=PHI_R;
    end
    disp(['   loop_count = ',num2str(k)]);
