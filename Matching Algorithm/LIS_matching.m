function[correct,num_matched,final_output,num_mismatch,final_mainstar] = LIS_matching(num_input_stars,mainstar,nearby_star_vector,catalog,catalog_ID,mapping_table,error_range,limit_matching_number)
        num_matched=0;
        correct=1;
    ...
    %======================matching=======================%
    %=======searching range=====%
    if floor((dis_first2main/0.01)) == 0
        searching_head = mapping_table(1,2);
        searching_end = mapping_table(1,3);
    elseif floor((dis_first2main/0.01)) > length(mapping_table(:,1))
        searching_head = mapping_table(length(mapping_table(:,1)),2);    %mapping table head
        searching_end = mapping_table(length(mapping_table(:,1)),3);     %mapping table end
    else
        searching_head = mapping_table(floor((dis_first2main/0.01)),2);    %mapping table head
        searching_end = mapping_table(floor((dis_first2main/0.01)),3);     %mapping table end
    end
    
    %=======vote==========%
    vote_list = zeros((searching_end - searching_head + 1),num_input_stars); %vote list
    search_count = 0;
    
    for i = searching_head:1:searching_end
        ...
        for j =1:1:(num_input_stars-2)
            ...
            if catalogue_search_count > catalog(i,3)-1
                catalogue_search_count = catalogue_search_count - 1;
                break;
            end
            if catalog(i,(2*j)+2) == 0 || catalog(i,(2*j)+3) == 0
                continue;
            end
            ori_flag = 0;
            jump_flag = 0;
                % if route is not match, search next data
            while VB(1,(2*j)-1) < catalog(i,(2*catalogue_search_count)+2) - error_range || VB(1,(2*j)-1) > catalog(i,(2*catalogue_search_count)+2) + error_range
                if  ori_flag == 0
                    ori = catalogue_search_count;
                    ori_flag = 1;
                end
                catalogue_search_count = catalogue_search_count + 1;
                if catalogue_search_count > catalog(i,3)-1      %catalog(i,3)-1
                    jump_flag = 1;
            ...
            if jump_flag == 1
                ...
                if VB(1,(2*j)-1) >= catalog(i,(2*catalogue_search_count)+2) - error_range && VB(1,(2*j)-1) <= catalog(i,(2*catalogue_search_count)+2) + error_range
                    if VB(1,(2*j)) >= catalog(i,(2*catalogue_search_count)+3) - error_range && VB(1,(2*j)) <= catalog(i,(2*catalogue_search_count)+3) + error_range
                        vote_list(search_count,1) = vote_list(search_count,1) + 1;
                        vote_list(search_count,2) = catalog_ID(searching_head + search_count - 1,4);
                        vote_list(search_count,j+2) = catalog_ID(searching_head + search_count - 1,catalogue_search_count +4);
                    end
                end
            end
        end
   ...
   candidate_number = 0;
    ...
    final_mainstar = 0;
    num_mismatch = 0;
    if catalog_ID(index_vote_list + searching_head - 1,1) ~= mainstar(4)
        final_output = zeros(1,num_input_stars);
        final_mainstar = catalog_ID(index_vote_list + searching_head - 1,1);
        correct = 0;
    else
        final_mainstar = catalog_ID(index_vote_list + searching_head - 1,1);
